...
for(let i = 0 ; i < inputArr.length ; i++ ){                
    ...
    if(firstChar == "-"){                                   //checking for options or path 
...
for(let i = 0 ; i < filesArr.length ; i++){
    ...
    if(fs.existsSync(filesArr[i]) == true){                 //checking if file path is correct or not
...
if(optionsArr.length == 0){
    ...
    if(optionsArr.includes("-n") == true && optionsArr.includes("-b") == true){     //checking for edge case if b and n exists togther
       
       let n1 = optionsArr.indexOf("-n");   
       let b1 = optionsArr.indexOf("-b");

        if( n1 > b1 ){                                         //comparaing which operation occurs first
            optionsArr.splice(n1,1);                                    
        } else {
            optionsArr.splice(b1,1); 
        }
    }
